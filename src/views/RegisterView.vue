  ...
  methods:{
    ...
    checkPassword() {
      if (this.password == '' || this.password == undefined) {
        ...
        this.passwordErr='密码长度只能是6到20之间';
      } else if (!/^[0-9a-zA-Z]+$/.test(this.password)) {
        this.passwordErr='密码只能试用数字和字母';
        ...
        if (this.phoneErr == '') {
          ...
          doGet('v1/sms/code/register', {phone:this.phone}).then(resp =>{
            if(resp && (resp.data.code == 1000 || resp.data.code == 1006)) {
              layx.msg('短信已经发送',{dialogIcon: 'success', position:'ct'})
            }
          })
        }
      }
    ...
    requestUserRegister() {
      ...
      if (this.phoneErr == '' && this.passwordErr == '' && this.codeErr == '') {
        if(this.agree) {
          ...
          layx.msg("请阅读注册服务协议", {dialogIcon: 'warn', position:'ct'});
        }
      }
    }
  }
      ...
      <div class="login-box">
        ...
        <form action="" id="register_Submit">
          <div class="alert-input">
            ...
            <div class="err">{{phoneErr}}

            </div>
            <p class="prompt_num"></p>
            <input type="password" placeholder="请输入6-20位英文和数字混合密码" v-model="password" @blue="checkPassword"
                   class="form-border user-pass"
                   autocomplete name="password">
            <div class="err">{{passwordErr}}</div>
            <p class="prompt_pass"></p>
            <div class="form-yzm form-border">
              <input class="yzm-write" type="text" name="" placeholder="输入短信验证码" v-model="code" @blue="checkCode">
              <input class="yzm-send" type="button" v-bind:value="yzmText" @click="requestSmsCode" id="yzmBtn" readonly="readonly">
